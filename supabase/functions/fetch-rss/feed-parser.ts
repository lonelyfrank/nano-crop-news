...
/**
 * Scarica e fa il parsing di un feed RSS 2.0 / Atom.
 *
 ...
 * enclosure/media per l'immagine), per evitare problemi di copyright.
 */
export async function fetchFeed(rssUrl: string, userAgent: string, timeoutMs: number): Promise<FeedItem[]> {
...
function parseItem(item: any): FeedItem | null {
  try {
    ...
    return { title, link, description, author, imageUrl, publishedAt }
  } catch {
    return null
  }
}
